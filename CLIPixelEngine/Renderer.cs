...
public class Renderer
{
  ...
  private Bitmap GetMap(string pathToMap)
  {
    return new(pathToMap);
  }
  ...
  public Task Draw()
  {
    ...
    for (var y = _startAtX; y < _endAtX; y++)
    {
      for (var x = _startAtY; x < _endAtY; x++)
      {
        var r = Map.GetPixel(x, y).R;
        var g = Map.GetPixel(x, y).G;
        var b = Map.GetPixel(x, y).B;
  ...
  {
    for (var x = 0; x < _sizeOfSprite; x++)
    for (var y = 0; y < _sizeOfSprite; y++)
    {
      ...
      {
        var spriteColor = entity.Sprite.GetPixel(entity.invertX ? 7 - x : 0 + x, y);
  ...
  private void DrawOverlay(Bitmap map, int startX, int startY)
  {
    ...
    {
      var overlay = Engine.overlays[overlayName].Image;
      for (var y = 0; y < Engine.camera.Fov.x * 2; y++)
      for (var x = 0; x < Engine.camera.Fov.y * 2; x++)
      {
        var spriteColor = overlay.GetPixel(x, y);
  ...
  {
    var posInInventory = new Dictionary<uint, int[]>
    {
      {2, new[] {14, 33}},
      {3, new[] {14, 33}},
      {4, new[] {14, 33}},
    ...
    var ring = equipment.sprite;

    for (var x = 0; x < _sizeOfSprite; x++)
    for (var y = 0; y < _sizeOfSprite; y++)
    {
      var spriteColor = ring.GetPixel(x, y);
